...
echo '<h2>Flightplan score for ' . $var1 . '</h2>';
...
if ($count >= 5) {
    ...
    echo '<table><tr><td>';
    echo ' Current attendence is: ' . round($totalattendance) . ' and so the score for the flightplan is: ' . $attendancescore;
    echo '<input type="hidden" name="attendance" value="' . $attendancescore . '">';
    echo '</td></tr><tr><td>';
// dropdown list for the quality of work score
    echo 'Enter Quality of Work/ Grades achived score: ';
...
echo '<h2>QCA and MTG Scores</h2>';
//form to add QCA


echo '<h2>QCA Score</h2>';
if ($qca > 0) {
    echo 'Current QCA score is: <br/>' . $qca;
} else {
    echo 'QCA not set';
}
echo '<form name="add_qca" action="process_qca.php?var1=' . $var1 . '&var2=' . $var2 . '&var3=' . $var3 . '&var4=' . $var4 . '" method="POST">';
echo 'Enter new QCA: <input type="text" size=5 name="qca_score" value="" /><br/>';
if ($qca > 0) {
    echo '<input name="submit_qca" type="submit" value="Change QCA">';
} else {
    echo '<input name="submit_qca" type="submit" value="Submit QCA">';
}
echo '</form>';
...
if ($mtg > 0) {
    echo '<input name="submit_mtg"  type="submit" value="Change MTG">';
} else {
    echo '<input name="submit_mtg" type="submit" value="Submit MTG">';
}
echo '</form>';

//echo 'QCA on entry score is: 46';
