...
if($action == 'updatetarget'){
	...
	if($catcount = count_records('ilp_post_category','module','ilptarget') > 0){
		echo '<div class="target-category-key"><p>Category Key:</p>';
		$targetcategories = get_records('ilp_post_category','module','ilptarget');
		foreach ($targetcategories as $targetcategory) {
			echo '<p class="target-category-'.$targetcategory->id.'" style="width:20px; margin-left: 20px">&nbsp;</p><p>'.$targetcategory->name.'</p>';
		}
