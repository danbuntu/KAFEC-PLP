...
class ilptarget_updatetarget_form extends moodleform {
    ...
    function definition() {
		...
		if(count_records('ilp_post_category','module','ilptarget')){
			$targetcategories = get_records('ilp_post_category','module','ilptarget');
			$options = array();
			foreach ($targetcategories as $targetcategory) {
				$options[$targetcategory->id] = $targetcategory->name;
			}
			$mform->addElement('select', 'category', get_string('categorydetails','ilptarget'), $options);
			if($targetpost > 0 && $report){
				$mform->setDefault('category', $report->category);
			}else{
...
class ilptarget_updatecomment_form extends moodleform {
    ...
    function definition() {
        ...
        $mform->addElement('htmleditor', 'commentpost', get_string('comment', 'ilptarget'));
        $mform->setType('commentpost', PARAM_RAW);
        $mform->addRule('commentpost', null, 'required', null, 'client');
        $mform->setHelpButton('commentpost', array('writing', 'richtext'), false, 'editorhelpbutton');
        if($commentid > 0 && $report){
            $mform->setDefault('commentpost', $report->commentpost);
        }
    ...
    function process_data($data) {
        ...
        if (!$report = get_record('ilptarget_comments', 'targetpost', $targetpost, 'id', $commentid)) {
			...
			$report->commentpost = $data->commentpost;
			$report->format = $data->format;
...
// Creates a form to add categories to the configuration options

class ilptarget_addcategory_form extends moodleform {
    ...
    function definition() {
        ...
        if($id > 0) {
			$category = get_record('ilp_post_category','id',$id);
			$mform->addElement('hidden', 'id', $category->id);
		}

		$mform->addElement('header', 'category', get_string('categorydetails', 'ilptarget'));
		...
		$mform->addElement('htmleditor', 'summary', get_string('categorydescription','ilptarget'), array('canUseHtmlEditor'=>'detect','rows'  => 20,'cols'  => 65));
		...
		if($id > 0 && $category){
			$mform->setDefault('summary', $category->summary);
        }
    ...
    function process_data($data) {
        ...
        if (!$category = get_record('ilp_post_category','id',$id)) {
            $category = new Object;
			$category->name = $data->name;
			$category->module = 'ilptarget';
			$category->status  = $data->status;
			$category->summary = $data->summary;

            $categoryinstance = insert_record('ilp_post_category', $category, true);
            ...
            $categoryinstance = update_record('ilp_post_category', $category, true);
