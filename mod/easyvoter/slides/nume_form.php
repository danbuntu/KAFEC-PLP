...
class nume_form extends moodleform {
    ...
    function definition() {
		...
		$mform =& $this->_form;
		if($sid>0){
			$mform->addElement('header', 'addedit', get_string('easyvoternumetype', 'easyvoter').' ('.get_string('easyvoteredit', 'easyvoter').')');
		}else{
			$mform->addElement('header', 'addedit', get_string('easyvoternumetype', 'easyvoter').' ('.get_string('easyvoteradd', 'easyvoter').')');
		}
    ...
    ///ADD ANSWER
        $mform->addElement('text', 'answer', get_string('easyvotercorrectanswer', 'easyvoter').'<br />'.get_string('easyvoterleaveempty', 'easyvoter'), array('size'=>'64'));
		$mform->setType('answer', PARAM_TEXT);
		
	//HIDDEN FIELDS
		$mform->addElement('hidden', 'id', $id);
		$mform->addElement('hidden', 'type', 'nume');
		if($sid>0){
...
//PERFORM EXTRA VALIDATION
    function validation($data){
		...
		if(!easyvoter_slideTypes($data['type'],$data['answer'])){
			$errors['answer'] = get_string('easyvotervalnumeint', 'easyvoter');
		}
