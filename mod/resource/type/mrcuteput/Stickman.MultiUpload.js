...
var MultiUpload = new Class(
{
	...
	addRow:function(){
		if( this.max == 0 || this.elements.length <= this.max ){
			...
			if(name != ''){
				var title = document.getElementsByName('multifiletitle')[0];
				var objItems = document.getElementById('id_items');
				var option = document.createElement('option');
				option.innerHTML  = title.value;
				option.value = current_element.element.name+':'+title.value;
	...
	initializeElement:function( element ){
		...
		var addbutton = document.getElementsByName('addmultifile')[0];
		$( addbutton )
