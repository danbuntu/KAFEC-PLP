<?php
	...
	function recursive_get_posts($post) {
		$children = get_records_select('forum_posts', "parent = " . $post->id);
		...
		$posting_user = get_record_select('user', "id = " . $post->userid);
		$return_array["username"] = $posting_user->username;
	...
	function array_for_forum_module($forum)
	{
		$return_array = array();
		$return_array["id"] = $forum->id;
		$return_array["name"] = $forum->name;
		...
		foreach($modules as $module) {
			...
			if($show_module) {
				...
				if($module->modname == "label") {
					...
					$new_module["instance"] = array_for_forum_module(get_record('forum', 'id', $module->instance));
					if($new_module["instance"]) {
						$return_array[] = $new_module;
					}
				}
