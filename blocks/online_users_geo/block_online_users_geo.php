<?php //$Id: block_online_users_geo.php,v 1.54.2.7 2009/11/20 03:08:59 andyjdavis Exp $
 ...
 * teacher roles.
 */
class block_online_users_geo extends block_base {
    function init() {
        $this->title = 'Online Users (GEO)';
    ...
    function get_content() {
        ...
        if (isset($CFG->block_online_users_geo_timetosee)) {
            $timetoshowusers = $CFG->block_online_users_geo_timetosee * 60;
        }
        ...
        if (!has_capability('block/online_users_geo:viewlist', $blockcontext)) {
            if (!$usercount = count_records_sql("SELECT COUNT(DISTINCT(u.id)) $from $where")) {
                $usercount = get_string("none");
            }
            $this->content->text = "<div class=\"info\">".get_string("periodnminutes","block_online_users_geo",$minutes).": $usercount</div>";
            return $this->content;
        }
            ...
            foreach ($users as $user) {
                ...
                if ($canshowicon and ($USER->id != $user->id) and  $user->username != 'guest') {  // Only when logged in and messaging active etc
                    $this->content->text .= '<div class="message"><a title="'.get_string('messageselectadd').'" href="'.$CFG->wwwroot.'/message/discussion.php?id='.$user->id.'" onclick="this.target=\'message_'.$user->id.'\';return openpopup(\'/message/discussion.php?id='.$user->id.'\', \'message_'.$user->id.'\', \'menubar=0,location=0,scrollbars,status,resizable,width=400,height=500\', 0);">'
                        .'<img class="iconsmall" src="'.$CFG->pixpath.'/t/message.gif" alt="'. get_string('messageselectadd') .'" /></a>';
                }
				
				$sql = "SELECT lat, lon, location FROM ".$CFG->prefix."nwkc_geolocation WHERE studentid = ".$user->id." AND timecheckedin > (UNIX_TIMESTAMP()-3600)";
				$locdata = get_record_sql($sql);
					...
					if ($CFG->$tmpstr) {	
						...
						if (count($tmparr)==4) {
							...
							$tempoutput.= 'if (($lat >= '.$tmparr[0].' && $lat <= '.$tmparr[2].') && ($lon >= '.$tmparr[1].' && $lon <= '.$tmparr[3].')) {
									$loccampus = "'.substr($CFG->$tmpstr, 0, 1).'";
								}';
							$condindex++;
						} // if (count($tmparr)==4)
					} // if ($CFG->$tmpstr)
				...
				if ($locdata) {
					$tmpstr = 'block_nwkc_geolocation_loc'.$locdata->location;
					if ($CFG->$tmpstr) {
						$this->content->text .= "<br><small>&nbsp;&nbsp;&nbsp;Location:&nbsp;".$CFG->$tmpstr."&nbsp;(".$loccampus.")</small>";
					}
				}
