...
<?php
    ...
    $bookmark = get_record('block_exabeporbooklink', 'id', $bookid, 'userid', $USER->id);
    if (!$bookmark) {
    	// it can also be shared to noone except to the userid of the current user
        $bookmark = get_record_sql("SELECT bf.*
                                    FROM {$CFG->prefix}block_exabeporbooklink bf
                                    LEFT JOIN {$CFG->prefix}block_exabeporsharlink bfs ON bf.id=bfs.bookid
                                    WHERE bf.shareall='0'
                                    AND bf.id='$bookid'
        ...
        if(!$bookmark) {
            ...
            $bookmark = get_record_sql("SELECT bf.*
                                        FROM {$CFG->prefix}block_exabeporbooklink bf
                                        LEFT JOIN (
                                          SELECT * FROM {$CFG->prefix}block_exabeporsharlink WHERE userid='{$USER->id}'
    ...
    if($deletecomment == 1) {
    	...
    	if(count_records("block_exabeporcommlink", "id", $commentid, "userid", $USER->id, "bookmarkid", $bookid) == 1) {
    		delete_records("block_exabeporcommlink", "id", $commentid, "userid", $USER->id, "bookmarkid", $bookid);
    	}
    	else {
	...
	if ($commentseditform->is_cancelled());
	    ...
	    switch ($action) {
	        case 'add':
	            do_add($fromform, $commentseditform, $courseid);
	            redirect("bookmark_view_external_link.php?courseid=$courseid&amp;bookid=$bookid&amp;original=$original");
	        break;
	    }
	}
	...
	show_comments($bookid,$courseid, $bookmark, $courseid);
    ...
    if($original==0) {
    	...
    	echo "<br /><a href='{$CFG->wwwroot}/blocks/exabis_eportfolio/view_external_links.php?courseid=$courseid'>".get_string("back","block_exabis_eportfolio")."</a><br /><br />";
    }
    else {
    ...
    function show_comments($bookmarkid, $courseid, $bookmark, $courseid) {
    	$comments = get_records("block_exabeporcommlink", "bookmarkid", $bookmarkid, 'timemodified DESC');
    	if($comments) {
    		foreach ($comments as $comment) {
    			block_exabis_eportfolio_print_comment($comment, 'bookmark_view_external_link.php',$courseid, $bookmark);
    		}
    	}
    }
	...
	function do_add($post, $blogeditform, $courseid) {
	    ...
	    if (insert_record('block_exabeporcommlink', $post)) {
	        add_to_log(SITEID, 'exabis_eportfolio', 'add', 'bookmark_view_external_link.php', $post->entry);
	    } else {
