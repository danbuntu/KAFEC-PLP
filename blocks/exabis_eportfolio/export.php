...
<?php
    ...
    require_once("{$CFG->dirroot}/blocks/exabis_eportfolio/minixml.inc.php");
	...
	$strexport = get_string("export","block_exabis_eportfolio");
   
    if ($courseid == SITEID) {
	 	print_header("$SITE->shortname: $strexport", $SITE->fullname,
		             "<a href=\"{$CFG->wwwroot}/blocks/exabis_eportfolio/view.php?id={$courseid}\">" . strip_tags(get_string("mybookmarks", "block_exabis_eportfolio")) . "</a> ->
		             <a href=\"{$CFG->wwwroot}/blocks/exabis_eportfolio/exportimport.php?courseid={$courseid}\">" . get_string("exportimport", "block_exabis_eportfolio") . "</a> ->
		             " . $strexport,
		             '', '', true);
	}
    else {
	 	print_header("$SITE->shortname: $strexport", $course->fullname,
    ...
    print_heading("<img src=\"{$CFG->wwwroot}/blocks/exabis_eportfolio/pix/export.png\" width=\"16\" height=\"16\" alt='".get_string("export", "block_exabis_eportfolio")."' /> " . $strexport) ;
...
function get_category_content(&$xmlElement, &$resources, $id, $name, $exportpath, $export_dir, $identifier, &$ridentifier) {
        global $USER, $CFG, $COURSE;
        $bookmarks = get_records_sql("select *".
                      " from {$CFG->prefix}block_exabeporbooklink ".
        ...
        if($bookmarks) {
        	foreach($bookmarks as $bookmark) {
        		...
        		$filecontent .= add_comments('block_exabeporcommlink', $bookmark->id);
        		$filecontent .= '</body>'."\n";
        		$filecontent .= '</html>'."\n";
        		...
        		create_item($xmlElement, 'ITEM-' . $identifier, spch(format_string($bookmark->name)), 'RES-' . $ridentifier);
        		$identifier++;
        		$ridentifier++;
        	}
        }
        
        $files = get_records_sql("select *".
              " from {$CFG->prefix}block_exabeporbookfile" .
              " where userid = $USER->id".
              " and category = $id".
              " order by name desc");
        ...
        if($files) {
        	foreach($files as $file) {
        		...
        		$filecontent .= add_comments('block_exabeporcommfile', $file->id);
        		$filecontent .= '</body>'."\n";
        		$filecontent .= '</html>'."\n";
        		...
        		file_put_contents($exportpath.$filepath, $filecontent);
        		create_ressource($resources, 'RES-' . $ridentifier, $filepath);
        		create_item($xmlElement, 'ITEM-' . $identifier, spch($file->name), 'RES-' . $ridentifier);
        		$identifier++;
        		$ridentifier++;
        	}
        ...
        $notes = get_records_sql("select *".
               " from {$CFG->prefix}block_exabepornote" .
               " where userid = $USER->id".
               " and category = $id".
               " order by name desc");
        ...
        if($notes) {
        	foreach($notes as $note) {
        		...
        		$filecontent .= add_comments('block_exabeporcommnote', $note->id);
        		$filecontent .= '</body>'."\n";
        		$filecontent .= '</html>'."\n";
        		...
        		file_put_contents($exportpath.$filepath, $filecontent);
        		create_ressource($resources, 'RES-' . $ridentifier, $filepath);
        		create_item($xmlElement, 'ITEM-' . $identifier, spch($note->name), 'RES-' . $ridentifier);
        		$identifier++;
        		$ridentifier++;
        	}
        }
    ...
    if ( $confirm ) {
    	...
    	//if($export == get_string("createexport", "block_exabis_eportfolio")) {
			...
			$userdescriptions = get_records_select("block_exabeporpers", "userid = '$USER->id'", '', '*', '', '1');
    		...
    		create_ressource($resources, 'RES-' . $ridentifier, $filepath);
    		create_item($desc_organization, 'ITEM-' . $identifier, spch(fullname($USER, $USER->id)), 'RES-' . $ridentifier);
    		$identifier++;
    		$ridentifier++;
		    ...
		    if ($owncats){
				...
				foreach ($owncats as $owncat){
    	    		echo "<li>".spch(format_string($owncat->name));
		    		unset($item);
		    		$i++;
		    		$item =& $organization->createChild('item');
		    		$item->attribute('identifier', sprintf('B%04d', $i));
		    		//$item->attribute('identifierref', 'R_S100001');
		    		$item->attribute('isvisible', 'true');
		    		$itemtitle =& $item->createChild('title');
					$itemtitle->text(spch($owncat->name));
					
					
					// get everything inside this category:
					get_category_content($item, $resources, $owncat->id, $owncat->name, $exportdir, 'data/', $identifier, $ridentifier);
		    	    ...
		    	    if($innerowncats) {
		    	    	...
		    	    	foreach ($innerowncats as $innerowncat){
    	    				echo "<li>".spch(format_string($innerowncat->name))."<br />";
				    		unset($subitem);
				    		$i++;
				    		$subitem =& $item->createChild('item');
				    		$subitem->attribute('identifier', sprintf('B%04d', $i));
				    		//$item->attribute('identifierref', 'R_S100001');
				    		$subitem->attribute('isvisible', 'true');
				    		$subitemtitle =& $subitem->createChild('title');
							$subitemtitle->text(spch($innerowncat->name));
							
							get_category_content($subitem, $resources, $innerowncat->id, $innerowncat->name, $exportdir, 'data/', $identifier, $ridentifier);
						echo "</li>";	
		    	    	}
    	    			echo "</ul>";
		    	    }
			...
			if(file_put_contents($exportdir . 'imsmanifest.xml', $parsedDoc->toString()) === false) {
	...
	echo '<form method="post" class="block_eportfolio_center" action="'.$CFG->wwwroot.'/blocks/exabis_eportfolio/export.php" >';
    echo '<fieldset>';
...
function add_comments($table, $bookmarkid) {
		$commentsContent = '';
		$comments = get_records($table, "bookmarkid", $bookmarkid);
		$i=1;
    	if($comments) {
