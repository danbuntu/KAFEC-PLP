...
<?php // $Id: share.php,v 1.1 2004/04/30 12:08:28 by guitarsawicki
    ...
    // Parameter for previous view (view_notes.php)
    ...
    $bookmark = get_record_sql("select b.*, bc.name AS cname, bc2.name AS cname_parent, c.fullname AS coursename".
                                 " from {$CFG->prefix}block_exabepornote b join {$CFG->prefix}block_exabeporcate bc on b.category = bc.id".
    ...
    if ($save_this == "ok"){
       ...
       update_record("block_exabepornote", $bookmark_update);
       
        delete_records("block_exabeporsharnote", "course", $courseid, "bookid", $bookid);
        ...
        if (is_array($sharethis)){
            foreach ($sharethis as $share_item) {
	            ...
	            insert_record("block_exabeporsharnote", $bookmark_shared);
            }
	    }
	    
	    redirect("view_notes.php?courseid=$courseid&sortkey=$sortkey&sortorder=$sortorder");
    }
    ...
    print_heading("<img src=\"{$CFG->wwwroot}/blocks/exabis_eportfolio/pix/notes.png\" width=\"16\" height=\"16\" alt='".get_string("strshare", "block_exabis_eportfolio")."' /> " . $strbookmarks . ": " . get_string("bookmarksnotes", "block_exabis_eportfolio")) ;
    ...
    print_simple_box( text_to_html(get_string("explainingsharenote","block_exabis_eportfolio")) , "center");
	...
	echo "<br /><form action=\"{$CFG->wwwroot}/blocks/exabis_eportfolio/sharenote.php\" method=\"post\" id=\"listing\">";
	...
	if($showall == 0) {
		...
		print_user_listing($courseid, $bookid, "block_exabeporsharnote",$course);
		echo "</fieldset>";
	}
	else {
	    ...
	    foreach($courses as $actcourse)
	    {
		    ...
		    print_user_listing($actcourse->id, $bookid, "block_exabeporsharnote",$actcourse);
			echo "</fieldset>";
		}
	}
	...
	if ($sortkey == ""){
        ...
        if($showall == 0)
        	echo "<br /><input type=\"button\" onclick=\"document.getElementById('listing').elements['showall'].value = '1';document.getElementById('listing').submit();\" value=\"".get_string("showallusers", "block_exabis_eportfolio")."\" />&nbsp;&nbsp;&nbsp;";
        else
            echo "<br /><input type=\"submit\" onclick=\"document.getElementById('listing').elements['showall'].value = '0';\" value=\"".get_string("showcourseusers", "block_exabis_eportfolio")."\" />&nbsp;&nbsp;&nbsp;";
