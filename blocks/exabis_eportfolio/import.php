...
<?php
    ...
    require_once("{$CFG->dirroot}/blocks/exabis_eportfolio/class.scormparser.php");
	...
	require_once("{$CFG->dirroot}/blocks/exabis_eportfolio/edit_form.php");
    ...
    if ($courseid == SITEID) {
		print_header("$SITE->shortname: $strimport", $SITE->fullname,
		...
		print_header("$SITE->shortname: $strimport", $course->fullname,
    ...
    print_heading("<img src=\"{$CFG->wwwroot}/blocks/exabis_eportfolio/pix/import.png\" width=\"16\" height=\"16\" alt='".get_string("import", "block_exabis_eportfolio")."' /> " . $strimport) ;
...
function import_user_description($file) {
	...
	if(($startDesc = strpos($content,  '<!--###BOOKMARK_PERSONAL_DESC###-->')) !== false) {
	   	...
	   	if(($endDesc = strpos($content, '<!--###BOOKMARK_PERSONAL_DESC###-->', $startDesc)) !== false) {
	        if(record_exists('block_exabeporpers', 'userid', $USER->id)) {
	        	$record = get_record('block_exabeporpers', 'userid', $USER->id);
	        	$record->description = block_exabis_eportfolio_clean_text(substr($content, $startDesc, $endDesc-$startDesc));
		        $record->timemodified = time();
		        if (! update_record('block_exabeporpers', $record)) {
		        ...
		        if (! insert_record('block_exabeporpers', $newentry)) {
	...
	if((($startUrl = strpos($content,  '<!--###BOOKMARK_EXT_URL###-->')) !== false)&&
	   (($startDesc = strpos($content, '<!--###BOOKMARK_EXT_DESC###-->')) !== false)) {
		   ...
		   (($endDesc = strpos($content, '<!--###BOOKMARK_EXT_DESC###-->', $startDesc)) !== false)) {
		    ...
		    if($new->id = insert_record('block_exabeporbooklink', $new)) {
			    get_comments($content, $new->id, 'block_exabeporcommlink');
			}
	   		else {
	   ...
	   (($startDesc = strpos($content,     '<!--###BOOKMARK_FILE_DESC###-->')) !== false)) {
		   ...
		   (($endDesc = strpos($content, '<!--###BOOKMARK_FILE_DESC###-->', $startDesc)) !== false)) {
		   	...
		   	if(is_file($linkedFilePath)) {
		    	...
		    	//$new->url          = str_replace($CFG->wwwroot, "", $_SERVER["HTTP_REFERER"]);
		   		...
		   		if ($new->id = insert_record('block_exabeporbookfile', $new)) {
		   			...
		   			if(make_upload_directory($destination, false)) {
						...
						if(copy($linkedFilePath, $destination . '/' . $destination_name)) {
							set_field("block_exabeporbookfile", "attachment", $destination_name, "id", $new->id);
						}
						else {
			    	...
			    	get_comments($content, $new->id, 'block_exabeporcommfile');
		   		}
		   		else {
					notify(get_string("couldntinsert", "block_exabis_eportfolio", $title));
		   		}
	   	...
	   	if((($endDesc = strpos($content, '<!--###BOOKMARK_NOTE_DESC###-->', $startDesc)) !== false) ) {
		    ...
		    if($new->id = insert_record('block_exabepornote', $new)) {
			    get_comments($content, $new->id, 'block_exabeporcommnote');
			}
	   		else {
				notify(get_string("couldntinsert", "block_exabis_eportfolio", $title));
	   		}
