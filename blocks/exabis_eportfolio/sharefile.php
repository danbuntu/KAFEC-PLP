...
<?php // $Id: share.php,v 1.1 2004/04/30 12:08:28 by guitarsawicki
    ...
    // Parameter for previous view (view_files.php)
    ...
    $bookmark = get_record_sql("select b.*, bc.name AS cname, bc2.name AS cname_parent, c.fullname AS coursename".
                                 " from {$CFG->prefix}block_exabeporbookfile b join {$CFG->prefix}block_exabeporcate bc on b.category = bc.id" .
    ...
    $strbookmarks = get_string("mybookmarks", "block_exabis_eportfolio");
       ...
       update_record("block_exabeporbookfile", $bookmark_update);
       
        delete_records("block_exabeporsharfile", "course", $courseid, "bookid", $bookid);        
        ...
        if (is_array($sharethis)){
            foreach ($sharethis as $share_item) {
	            ...
	            insert_record("block_exabeporsharfile", $bookmark_shared);
            }
        }

	    redirect("view_files.php?courseid=$courseid&sortkey=$sortkey&sortorder=$sortorder");
    }
    ...
    print_heading("<img src=\"{$CFG->wwwroot}/blocks/exabis_eportfolio/pix/files.png\" width=\"16\" height=\"16\" alt='".get_string("strshare", "block_exabis_eportfolio")."' /> " . $strbookmarks . ": ". get_string("files")) ;
    ...
    print_simple_box( text_to_html(get_string("explainingsharefile","block_exabis_eportfolio")) , "center");
	...
	echo "<br /><form action=\"{$CFG->wwwroot}/blocks/exabis_eportfolio/sharefile.php\" method=\"post\" id=\"listing\">";
	...
	if($showall == 0) {
		...
		print_user_listing($courseid, $bookid, "block_exabeporsharfile",$course);
		echo "</fieldset>";
	}
	else {
	    ...
	    foreach($courses as $actcourse)
	    {
		    ...
		    print_user_listing($actcourse->id, $bookid, "block_exabeporsharfile",$actcourse);
			echo "</fieldset>";
		}
	}
