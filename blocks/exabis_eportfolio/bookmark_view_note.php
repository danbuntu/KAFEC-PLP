...
<?php
    ...
    $bookmark = get_record('block_exabepornote', 'id', $bookid, 'userid', $USER->id);
    if (!$bookmark) {
    	// it can also be shared to noone except to the userid of the current user
        $bookmark = get_record_sql("SELECT bf.*
                                    FROM {$CFG->prefix}block_exabepornote bf
                                    LEFT JOIN {$CFG->prefix}block_exabeporsharnote bfs ON bf.id=bfs.bookid
                                    WHERE bf.shareall='0'
                                    AND bf.id='$bookid'
        ...
        if(!$bookmark) {
            ...
            $bookmark = get_record_sql("SELECT bf.*
                                        FROM {$CFG->prefix}block_exabepornote bf
                                        LEFT JOIN (
                                          SELECT * FROM {$CFG->prefix}block_exabeporsharnote WHERE userid='{$USER->id}'
    ...
    if($deletecomment == 1) {
    	...
    	if(count_records("block_exabeporcommnote", "id", $commentid, "userid", $USER->id, "bookmarkid", $bookid) == 1) {
    		delete_records("block_exabeporcommnote", "id", $commentid, "userid", $USER->id, "bookmarkid", $bookid);
    	}
    	else {
	...
	if ($commentseditform->is_cancelled());
	    ...
	    switch ($action) {
	        case 'add':
	            ...
	            redirect("bookmark_view_note.php?courseid=$courseid&amp;bookid=$bookid&amp;original=$original");
	        break;
	    }
	}
    ...
    if($original==0) {
    	...
    	echo "<br /><a href='{$CFG->wwwroot}/blocks/exabis_eportfolio/view_notes.php?courseid=$courseid'>".get_string("back","block_exabis_eportfolio")."</a><br /><br />";
    }
    else {
    ...
    function show_comments($bookmarkid, $courseid, $bookmark) {
    	...
    	if($comments) {
    		foreach ($comments as $comment) {
    			block_exabis_eportfolio_print_comment($comment, 'bookmark_view_note.php',$courseid, $bookmark);
    		}
    	}
    }
	...
	function do_add($post, $blogeditform, $courseid) {
	    ...
	    if (insert_record('block_exabeporcommnote', $post)) {
	        add_to_log(SITEID, 'exabis_eportfolio', 'add', 'bookmark_view_note.php', $post->entry);
	    } else {
