...
<?php
    ...
    //require_once("{$CFG->dirroot}/blocks/exabis_eportfolio/categories_edit_form.php");
    ...
    print_heading("<img src=\"{$CFG->wwwroot}/blocks/exabis_eportfolio/pix/categories.png\" width=\"16\" height=\"16\" alt='".get_string("viewcategories", "block_exabis_eportfolio")."' /> " . $strbookmarks) ;
            ...
            switch ( $cataction )
            {
                ...
                case "edit":
                	if(($editid > 0) && ($editrecord = get_record("block_exabeporcate", "id", $editid, "userid", $USER->id)))
                	{
			            ...
			            ?><div class="block_eportfolio_center"><form method="post" action="<?php echo $CFG->wwwroot; ?>/blocks/exabis_eportfolio/viewcategories.php?courseid=<?php echo $courseid; ?>&amp;edit=1">
				          <fieldset>
                ...
                case "delete":
	                if($catconfirm==1) {
			        	...
			        	echo '<br />';
			        	notice_yesno(get_string("deletecategroyconfirm", "block_exabis_eportfolio"), 'viewcategories.php', 'viewcategories.php', $optionsyes, $optionsno, 'post', 'get');
			        	print_footer();
			        	die;
			        }
	                    ...
	                    if($delid > 0)
	                    {
		                    ...
		                    {
		                    	if ($entries = get_records_select('block_exabeporbookfile', 'category='.$delid, '', 'id')) {
							        foreach ($entries as $entry) {
							        	delete_records('block_exabeporsharfile', 'bookid', $entry->id);
							        }
							    }
							    delete_records('block_exabeporbookfile', 'category', $delid);
							    
		                    	if ($entries = get_records_select('block_exabepornote', 'category='.$delid, '', 'id')) {
							        foreach ($entries as $entry) {
							        	delete_records('block_exabeporsharnote', 'bookid', $entry->id);
							        }
							    }
							    delete_records('block_exabepornote', 'category', $delid);
							    
		                    	if ($entries = get_records_select('block_exabeporbooklink', 'category='.$delid, '', 'id')) {
							        foreach ($entries as $entry) {
							        	delete_records('block_exabeporsharlink', 'bookid', $entry->id);
							        }
							    }
							    delete_records('block_exabeporbooklink', 'category', $delid);
    ...
    if($edit == 1) {
    	...
    	if ( $outer_categories ) {
    		...
    		foreach ($outer_categories as $curcategory ) {
    			...
    			echo ' <a href="'.$CFG->wwwroot.'/blocks/exabis_eportfolio/viewcategories.php?cataction=edit&amp;sesskey=' . sesskey() . '&amp;catconfirm=1&amp;courseid='.$courseid.'&amp;editid='.$curcategory->id.'&amp;edit=1"><img src="'.$CFG->pixpath.'/t/edit.gif" width="11" height="11" alt="'.get_string("edit").'" /></a>';
    				
    			if($count_inner_categories == 0) {
    				echo ' <a href="'.$CFG->wwwroot.'/blocks/exabis_eportfolio/viewcategories.php?cataction=delete&amp;sesskey=' . sesskey() . '&amp;catconfirm=1&amp;courseid='.$courseid.'&amp;delid='.$curcategory->id.'&amp;edit=1"><img src="'.$CFG->pixpath.'/t/delete.gif" width="11" height="11" alt="'.get_string("delete").'" /></a>';
    			}
    			echo '</td>';
    			echo '<td valign="top">';
    			if($inner_categories) {
    				foreach ($inner_categories as $innercurcategory ) {
    					...
    					echo '<a href="'.$CFG->wwwroot.'/blocks/exabis_eportfolio/viewcategories.php?cataction=edit&amp;sesskey=' . sesskey() . '&amp;catconfirm=1&amp;courseid='.$courseid.'&amp;editid='.$innercurcategory->id.'&amp;edit=1"><img src="'.$CFG->pixpath.'/t/edit.gif" width="11" height="11" alt="'.get_string("edit").'" /></a>';
		          		echo '<a href="'.$CFG->wwwroot.'/blocks/exabis_eportfolio/viewcategories.php?cataction=delete&amp;sesskey=' . sesskey() . '&amp;catconfirm=1&amp;courseid='.$courseid.'&amp;delid='.$innercurcategory->id.'&amp;edit=1"><img src="'.$CFG->pixpath.'/t/delete.gif" width="11" height="11" alt="'.get_string("delete").'" /></a>';
		          		echo '<br />';
		          	}
		        }
		        
		      	echo '<form method="post" action="'.$CFG->wwwroot.'/blocks/exabis_eportfolio/viewcategories.php?courseid='.$courseid.'&amp;edit=1">';
		      	echo '<fieldset>';
    	...
    	echo '<tr>';
		echo '<td valign="top" style="text-align:center" colspan="2"><form method="post" action="'.$CFG->wwwroot.'/blocks/exabis_eportfolio/viewcategories.php?courseid='.$courseid.'"><fieldset><input type="submit" name="submit" value="'.get_string("endedit", "block_exabis_eportfolio").'" /><input type="hidden" name="sesskey" value="' . sesskey() . '" /></fieldset></form></td>';
		echo '</tr>';
		echo '</table></div>';
    }                 
    else {            
