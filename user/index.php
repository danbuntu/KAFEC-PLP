        ...
        if ($userlist)  {
            ...
            while ($user = rs_fetch_next_record($userlist)) {
                ...
                if ($piclink = ($USER->id == $user->id || $contextcanviewdetails || $usercontextcanviewdetails)) {
                    ...
                    $profilelink = '<strong><a href="'.$CFG->wwwroot.'/user/view.php?id='.$user->id.'&amp;course='.$course->id.'">'.fullname($user).'</a></strong> <a href="'.$CFG->wwwroot.'/blocks/ilp/view.php?courseid=1&id='. $user->id .'">View PLP</a>';
