<?php
{
    ...
    print_header($strtitle,$strtitle,"<A HREF=\"$CFG->wwwroot/course/view.php?id=$course->id\">$course->shortname</A> -> <a href=\"index.php?id=$course->id\">".get_string("participants")."</a> -> ".$strtitle);
    ...
    if (!empty($messagebody) && !$edit && !$deluser && ($preview || $send)) {
        if (count($SESSION->emailto[$id])) {
            if ($preview) {
                echo '<form method="post" action="sendemail.php" style="margin: 0 20px;">
...
';
				...
				//	$mailto = $mailto && $SESSION->emailto[$id];
                //}
            } elseif ($send) {
                $good = 1;
                $teachers = array();
                ...
                if ($good) {
                    ...
                    echo '<a href="mailto:'.$mailto.'">Click here to email them</a>';
