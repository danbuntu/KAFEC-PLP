...
$query = "SELECT * FROM mdl_user WHERE id='" . $moodleId . "'";
//    echo $query;
...
<tr>
    <td colspan="5">
        <?php echo '<h1>Apprentice Report for ', $firstname, ' ', $lastname, '</h1>'; ?>
    </td>
    <td colspan="2">
...
$query = "SELECT * FROM apprentice_users WHERE learner_ref='$learnerRef' AND course_id='$courseCode'";
//echo $query;
$result = mysql_query($query);
while ($row = mysql_fetch_assoc($result)) {
...
<tr>
    <td>
        <form name="apprentice" method="Post" action="process_form.php">
            <select name="center">
...
<table style="width: 100%; text-align:center;">
    <tr>
        ...
        $query = "SELECT * FROM apprentice_units WHERE courseid='$employId'";
//        echo $query;
...
<?php
        ...
        foreach ($units as $item) {
            $queryUnit = "SELECT * FROM apprentice_user_units WHERE unit_id='$item' AND user_id='$learnerRef'";
            //        echo $queryUnit;
...
foreach ($resultAtt as $item) {
   if($item->Course_code == $courseCode){
      echo '<table style="text-align: center;">';
       echo '<tr><th colspan="3">Attendance for ' , $courseCode , '</th></tr>';
       echo '<tr><th>Possbile Attendance</th><th>Actual Attendance</th><th>Attendance as a Percentage</th></tr>';
...
$query = "SELECT * FROM apprentice_effectiveness";
$result = mysql_query($query);
...
<table style="width: 100%;">
    <tr>
        <th colspan="<?php echo $count2; ?>">Personal Effectiveness [1=Outstanding, 2=Good, 3=Satisfactory,
            4=Unsatisfactory]
        </th>
    </tr>
...
//    print_r($effectivenessTitles); 
    echo  '<tr><th>Measure</th>';
    foreach ($effectivenessTitles as $item) {
        //        echo $item['id'];
        // get previous score
        $queryPrev = "SELECT * FROM apprentice_effectiveness_user JOIN apprentice_effectiveness ON apprentice_effectiveness_user.effect_id=apprentice_effectiveness.id WHERE learner_ref='$learnerRef' AND effect_id='" . $item['id'] . "' ORDER BY date DESC LIMIT 1";
        echo $queryPrev;
        echo 'terst';
        ...
        while ($rowPrev = mysql_fetch_assoc($resultPrev)) {
            ...
            echo '<th class="unit">1', $item['name'], '</br>Previous: ' . $rowPrev['effectiveness_score'] . '</br><select name="effectiveness[]"/><option>--Select--</option><option>1</option><option>2</option><option>3</option><option>4</option></select></th>';
...
<table style="width: 100%;">
    <tr>
        <th style="width: 50%;">Last PLP Progress Review for this Course</th>
    </tr>
    <tr>
        <td style="width: 50%;">
<?php
$query = "SELECT * FROM mdl_ilpconcern_posts WHERE format='1' AND status='0' AND targetcourse='$courseId' AND setforuserid='$moodleId' ORDER BY timemodified desc LIMIT 1";
    $result = mysql_query($query);
//echo $query;
            while ($row = mysql_fetch_assoc($result)) {
...
<table width="100%">
    <tr>
        ...
        <th colspan="4">Training Centre Rep Details</th>
        <th colspan="4">Period of the Report</th>
    </tr>
    <tr>
