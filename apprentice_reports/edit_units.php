...
if (isset($_POST['deleteUnit'])) {
    $query = "DELETE FROM apprentice_units WHERE id='" . $_POST['id'] . "'";
    echo $query;
    mysql_query($query);
}

if (isset($_POST['updateUnit'])) {
    $query = "UPDATE apprentice_units set name='" . $_POST['unitName'] . "', description='" . $_POST['unitDescription'] . "' WHERE id='" . $_POST['unitId'] . "'";
    echo $query;
    mysql_query($query);
}

if (isset($_POST['add_new_unit'])) {
    $query = "INSERT INTO apprentice_units (name, description, courseid, markid) VALUES ('" . $_POST['unitName'] . "','" . $_POST['unitDescription'] . "','" . $_SESSION['courseid'] . "','" . $_POST['marking'] . "')";
    echo $query;
    mysql_query($query);
}
...
$query = "SELECT * FROM apprentice_courses WHERE id='" . $_SESSION['courseid'] . "'";
...
?>
 <h1>All units for Course <?php echo $courseName ?> </h1>
    <a href="edit_effectiveness.php">Back to the main screen</a>

    <?php


$query = "SELECT apprentice_units.id, name, courseid, description, markid, apprentice_marks.id as markid2, type
FROM apprentice_units JOIN apprentice_marks ON apprentice_units.markid=apprentice_marks.id
...
while ($row = mysql_fetch_assoc($result)) {
    ...
    <tr>
        ...
        <td>
            ...
            $queryMark = "SELECT * FROM apprentice_marks";
...
// Get possbile marking scehemes
                    $query = "SELECT * FROM apprentice_marks";
                    $result = mysql_query($query);
                    echo '<select name="marking">';
                        while ($row = mysql_fetch_assoc($result)) {
