...
// check for a value
checkDropdown($center, 'Training Centre');
...
$query = "SELECT * FROM apprentice_users WHERE learner_ref='" . $ID . "' AND course_id='" . $courseCode . "'";
echo $query;
...
if ($num_rows >= 1) {
    ...
    $query = "INSERT INTO apprentice_users (moodle_id, learner_ref, rep_name, employer_id, training_centre, programme_start_date, officer, course_id, employer)
...
for ($i = 0; $i < count($_POST['unitId']); $i++) {
    ...
    $queryCheck = "SELECT * FROM apprentice_user_units WHERE unit_id='" . $unitId . "' AND user_id='" . $ID . "'";
    echo $queryCheck;
    ...
    if ($num_rows_check >= 1) {
        ...
        $query = "UPDATE apprentice_user_units SET target='$target', evidence='$evidence' WHERE unit_id='" . $unitId . "' AND user_id='" . $ID . "'";
        echo $query;
        mysql_query($query);
    } else {
        echo 'the record doesn\'t exist creating';
        $query = "INSERT INTO apprentice_user_units (unit_id, user_id, target, evidence) VALUES ('$unitId', '$ID', '$target', '$evidence')";
        echo $query;
        mysql_query($query);
    }


    //    $query = "INSERT INTO apprentice_effectiveness_user (learner_ref, effect_id, comment, date) VALUES ($ID, $effectivenessId, $effectiveness, $effectivenessComments $todaysDate)";
    //echo $query;
}
...
for ($i = 0; $i < count($_POST['effectivenessId']); $i++) {
    ...
    $query = "INSERT INTO apprentice_effectiveness_user (learner_ref, effect_id, effectiveness_score, comment, date) VALUES ('$ID', '$effectivenessId', '$effectiveness', '$effectivenessComments', '$todaysDate')";
    echo $query;
    mysql_query($query);
}


function checkDropdown($value, $test)
{
    if ($value == '--Select--') {
        echo 'You must enter a value for ' , $test;
             echo '<meta http-equiv="refresh" content="3; url=/apprentice_reports/view.php">';
        break;
